...
/**
 * Approve application and generate Supply Order (FR-ADMIN-002)
 * @param {string} applicationId - Application ID
 * @param {string} adminId - Admin user ID
 */
exports.approveApplication = async (applicationId, adminId) => {
  ...
  const oldStatus = application.status;
  application.status = 'approved_payment_required';
  ...
  await ApplicationStatusHistory.create({
    ...
    changedBy: adminId,
    notes: `تمت الموافقة - المبلغ المطلوب: ${price.price} جنيه`,
  });
  ...
  await Notification.create({
    ...
    message: `تمت الموافقة على طلبك رقم ${application.applicationNumber}. المبلغ المطلوب: ${price.price} جنيه مصري. رقم أمر التوريد: ${supplyOrderId}`,
    applicationId: application.id,
  });
